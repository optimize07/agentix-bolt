...
export function BulkAssetUploader({ projectId, groupId, onComplete }: BulkAssetUploaderProps) {
    ...
    const newFiles: UploadFile[] = selectedFiles
      .filter((file) => {
        ...
        if (file.size > maxSize) {
          toast.error(`${file.name}: File too large (max ${type === "video" ? "50MB" : "20MB"})`);
          return false;
        }
  ...
  const generateTags = async (title: string, type: string): Promise<string[]> => {
    try {
  ...
  const uploadFile = async (uploadFile: UploadFile): Promise<UploadFile> => {
    try {
      ...
      // Generate tags
      const assetTitle = uploadFile.file.name.replace(/\.[^/.]+$/, "");
      const tags = await generateTags(assetTitle, uploadFile.type);
      ...
      if (uploadFile.type === "image" || uploadFile.type === "video") {
        assetData.thumbnail_url = publicUrl;
      }
  ...
  const handleUploadAll = async () => {
    ...
    try {
      ...
      if (errorCount === 0) {
        ...
        toast.warning(`Uploaded ${successCount} asset${successCount !== 1 ? "s" : ""}, ${errorCount} failed`);
      }
    } catch (error) {
      ...
      {files.length === 0 && (
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">
            Images & Documents: up to 20MB • Videos: up to 50MB
          </p>
        </div>
      )}
    </div>
