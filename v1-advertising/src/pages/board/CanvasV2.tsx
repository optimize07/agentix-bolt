...
function CanvasV2Inner() {
  ...
  const initialNodes: Node[] = useMemo(() => {
      ...
      .map((block) => {
        ...
        if (block.group_id) {
          if (validGroupIds.has(block.group_id)) {
            // Find parent group to check if position needs conversion
            const parentGroup = blocks.find(b => b.id === block.group_id);
            if (parentGroup) {
              // Check if position looks absolute (larger than reasonable relative position)
              // If position is greater than parent's position, it's likely stored as absolute
              const looksAbsolute = block.position_x > parentGroup.width || 
                                    block.position_y > parentGroup.height ||
                                    (block.position_x > 500 && block.position_x > parentGroup.position_x);
              
              if (looksAbsolute) {
                // Convert to relative position
                nodeConfig.position = {
                  x: Math.max(20, block.position_x - parentGroup.position_x),
                  y: Math.max(20, block.position_y - parentGroup.position_y)
                };
              }
            }
    ...
    // CRITICAL: Sort so parent groups appear before their children
    // ReactFlow requires parents to exist before children reference them via parentNode
    return mappedNodes.sort((a, b) => {
      // Groups without parents come first
      if (a.type === 'group' && b.type !== 'group') return -1;
      if (b.type === 'group' && a.type !== 'group') return 1;
      
      // Among non-groups, nodes without parentNode come before nodes with parentNode
  ...
  useEffect(() => {
    ...
    if (hasLoadedData) {
      ...
      // Double-check for orphaned parent references before setting nodes
      const validGroupIds = new Set(initialNodes.filter(n => n.type === 'group').map(n => n.id));
      const sanitizedNodes = initialNodes.map(node => {
        if (node.parentNode && !validGroupIds.has(node.parentNode)) {
          console.warn(`[Canvas] Removing orphaned parentNode ${node.parentNode} from node ${node.id}`);
          const { parentNode, extent, ...rest } = node;
          return rest;
        }
        return node;
      }).sort((a, b) => {
        // CRITICAL: Ensure groups come before their children after sanitization too
        if (a.type === 'group' && b.type !== 'group') return -1;
        if (b.type === 'group' && a.type !== 'group') return 1;
        const aHasParent = !!(a as any).parentNode;
        const bHasParent = !!(b as any).parentNode;
  ...
  // Callback to create an image block on canvas (for Quick Batch Generator)
  const handleCreateImageBlock = useCallback(async (imageUrl: string, position?: { x: number; y: number }) => {
    if (!projectId) return;
    ...
    try {
      ...
      const newNode: Node = {
        ...
        data: { 
          ...
          onEdit: () => {
            setNodes((currentNodes) => {
              const node = currentNodes.find(n => n.id === newBlock.id);
              if (node) {
                setEditingBlock(node);
                setEditDialogOpen(true);
              }
      ...
      saveState([...nodes, newNode], flowEdges);
  ...
  }, [projectId, addBlock, setNodes, saveState, nodes, flowEdges]);
  ...
  const handleCreateImageBlockRef = useRef(handleCreateImageBlock);
  
  useEffect(() => {
    handleUnlinkRef.current = handleUnlinkFromGroup;
    handleUpdateNodeDataRef.current = handleUpdateNodeData;
    handleCreateImageBlockRef.current = handleCreateImageBlock;
  });

  // Inject callbacks into nodes only once on mount
  useEffect(() => {
    setNodes((currentNodes) => {
      // Build set of valid group IDs from current nodes
      const validGroupIds = new Set(currentNodes.filter(n => n.type === 'group').map(n => n.id));
      
      return currentNodes.map((node) => {
        // Clean up orphaned parentNode references
        const hasValidParent = !node.parentNode || validGroupIds.has(node.parentNode);
        
        return {
          ...node,
          // Remove parentNode if it no longer exists
          ...(hasValidParent ? {} : { parentNode: undefined, extent: undefined }),
          data: {
            ...node.data,
            ...
            // For chat nodes: inject image block creation callback and position
            ...(node.type === 'chat' ? {
              onCreateImageBlock: (imageUrl: string, pos?: { x: number; y: number }) => handleCreateImageBlockRef.current(imageUrl, pos),
              nodePosition: node.position,
            } : {}),
            // Clean up orphaned group flags
            ...(hasValidParent ? {} : { isGroupChild: false, parentGroupId: undefined }),
          },
        };
      });
    });
  ...
  const handleSelectGroup = useCallback(
    async (groupId: string, contentType: string) => {
      ...
      try {
        ...
        // CRITICAL: Create the parent group FIRST so it exists when children reference it
        ...
        // Now create child blocks with group_id and relative positions
        ...
        for (let i = 0; i < items.length; i++) {
          ...
          const relativeX = padding + col * (blockWidth + spacing);
          const relativeY = padding + row * (blockHeight + spacing);
          ...
          // Update block with group_id immediately after creation
          await supabase
            .from("canvas_blocks")
          ...
          // Create child node with parentNode reference
          childNodes.push({
            id: newBlock.id,
            type: newBlock.type,
            position: { x: relativeX, y: relativeY },
        ...
        // Add nodes in correct order: group first, then children
        const currentNodes = nodes;
        const currentEdges = flowEdges;
