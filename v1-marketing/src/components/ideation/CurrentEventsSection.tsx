...
interface CurrentEventsSectionProps {
  events: CurrentEvent[];
  onUseEvent: (event: CurrentEvent) => void;
  isLoading?: boolean;
}
...
export function CurrentEventsSection({ events, onUseEvent, isLoading }: CurrentEventsSectionProps) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        {events.length === 0 ? (
          <p className="text-sm text-muted-foreground">Generate ideas to see current events</p>
        ) : (
          events.map((event, index) => (
            ...
            >
              ...
              <div className="bg-muted/30 rounded p-2 mb-2">
                <p className="text-xs"><span className="text-primary font-medium">Angle:</span> {event.angle}</p>
              </div>
              <div className="flex items-center justify-between">
                <p className="text-xs text-muted-foreground line-clamp-1">
                  <span className="font-medium">Why:</span> {event.relevance}
                </p>
                <Button 
                  size="sm" 
                  variant="ghost" 
