...
import { Maximize2, Download, Upload, Trash2, Settings, Plus, PlusCircle, GripVertical, Sparkles, Undo, Redo, ArrowUpDown, Paintbrush, X, MoreVertical } from "lucide-react";
...
const DataTable = ({ data, config, title, isEditMode = true, onCellChange, onCellClick, activeCell, onDeleteRow, onImportData, onDelete, onOpenConfig, onUpdateConfig, onAskAi, onTitleChange, cellHistory: externalHistory, historyIndex: externalHistoryIndex, onHistoryChange }: DataTableProps) => {
  ...
  const parseCSV = (text: string): any[][] => {
    ...
    if (emptyHeaderCount / headers.length > 0.5) {
      throw new Error('More than 50% of headers are empty. Please provide valid column names.');
    }

    // Auto-generate column names for empty headers
    result[0] = headers.map((header, idx) => {
      if (!header || header.trim() === '') {
        return `Column ${String.fromCharCode(65 + idx)}`;
      }
      // Sanitize special characters but keep spaces and common punctuation
      return header.replace(/[^\w\s\-_.]/g, '').trim() || `Column ${String.fromCharCode(65 + idx)}`;
    });
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (event) => {
        try {
          ...
          const autoGeneratedHeaders = rows[0].filter((h: string) => h.startsWith('Column '));
          if (autoGeneratedHeaders.length > 0) {
            toast.info(`Auto-generated ${autoGeneratedHeaders.length} column name(s): ${autoGeneratedHeaders.join(', ')}`);
          }
          
          if (onImportData && rows.length > 0) {
            onImportData(rows, showAiSuggestions);
            toast.success(`Imported ${rows.length - 1} rows with ${rows[0].length} columns`);
          }
        } catch (error) {
          toast.error(error instanceof Error ? error.message : 'Failed to import CSV. Please check the file format.');
        }
      };
  ...
  // Pure fallback - UnifiedCanvas is the single source of truth
  const baseData =
    data && data.length > 0
      ? data
  ...
  const handleAddColumn = () => {
    ...
    console.log('AddColumn: before', {
      baseCols: baseData[0]?.length,
      headersBefore: baseData[0],
    });

    const currentColumnCount = baseData[0]?.length || 0;
    const newColumnName = `Column ${String.fromCharCode(65 + currentColumnCount)}`;
    
    const updatedData = baseData.map((row, index) => {
      if (index === 0) {
    ...
    console.log('AddColumn: after', {
      newCols: updatedData[0]?.length,
      headersAfter: updatedData[0],
    });
  ...
  const processedData = useMemo(() => {
    return applyColumnConfig(applySorting(applyFilters(baseData)));
  }, [baseData, config?.filters, config?.sorting, config?.visibleColumns]);
  ...
  return (
    <Card className="h-full flex flex-col group relative border-0 shadow-2xl backdrop-blur-xl bg-background/40">
      <TooltipProvider>
        ...
        <div className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/40 shadow-lg">
          <div className="flex items-center gap-3 px-4 py-2.5">
            ...
            {/* Spacer to push selection tools to the right */}
      ...
      <CardContent className="p-0 flex-1 relative">
        {/* Single scroll container - handles both horizontal and vertical scrolling with always-visible scrollbars */}
        <div className="absolute inset-0 overflow-scroll sheet-scroll-container">
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <table className="border-collapse table-fixed w-max min-w-full">
              ...
              <thead className="sticky top-0 z-20 bg-background border-b border-border/40">
                <tr>
                  <th 
                    style={{ width: `${rowNumbersWidth}px`, minWidth: `${rowNumbersWidth}px` }}
                    className="sticky left-0 px-1 py-2 text-xs text-center font-mono bg-muted border-r border-border/40 z-10 relative"
                  >
                    #
                    {isEditMode && (
                      <div 
                  ...
                  <SortableContext items={processedData.length > 0 ? processedData[0].map((_, idx) => `col-${idx}`) : []}>
                    {processedData.length > 0 && processedData[0].map((cell, colIndex) => {
                      const headerName = headers[colIndex];
                      ...
                      return (
                        ...
                        >
                          {isEditMode && (
                            <div 
                              className="absolute right-0 top-0 bottom-0 w-1.5 bg-border/40 hover:bg-primary/60 cursor-col-resize transition-colors z-10"
                              onMouseDown={(e) => startColumnResize(e, headerName)}
                            />
                          )}
                        </SortableHeader>
                      );
              ...
              <tbody>
                <SortableContext items={processedData.slice(1).map((_, idx) => `row-${idx + 1}`)}>
                  {processedData.slice(1).map((row, arrayIndex) => {
                    const rowIndex = arrayIndex + 1;
                    return (
                      <SortableRow 
                        key={rowIndex} 
                        rowIndex={rowIndex}
                  ...
                  className={`text-sm border-b border-r border-border/40 relative bg-card overflow-hidden ${
                    isEditing
